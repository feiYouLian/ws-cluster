...
class WsClient {
    ...
    login(id) {
        ...
        this.conn.onopen = (evt) => {
            ...
            // join groups while reconnect
            if (this.groups.size > 0 && this.reconnectTimes > 0) {
                var groups = []
                this.groups.forEach(g => groups.push(g))
                this.joinGroups(groups, true)
            }
    ...
    joinGroups(groups, force) {
        ...
        if (!force) {
            groups = groups.filter(group => !this.groups.has(group))
        }
        if (groups.length == 0) {
            return
        }
...
class MessageHeader {
    ...
    // 解码 ByteBuffer to MessageHeader
    decode(buf) {
...
class ByteBuffer {
    ...
    putString(value) {
        let strLen = getByteLength(value)
...
const MsgUtils = {
    ...
    decode: (uint8array) => {
        try {
            let buf = new ByteBuffer(uint8array)
